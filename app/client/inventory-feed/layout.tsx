...
import {
  C_ALL_INVENTORY_ROUTE,
  C_INVENTORY_FEED_DASHBOARD_ROUTE,
...
const links = [
  ...
  {
    path: C_INVENTORY_FEED_DASHBOARD_ROUTE,
    label: "Dashboard",
    icon: LayoutDashboard,
  },
  {
    path: C_ALL_INVENTORY_ROUTE,
    label: "All Inventory",
    icon: Database,
  },
];
...
export default function InventoryFeedLayout({
  ...
  const isActive = (path: string) => {
    if (path === C_INVENTORY_FEED_LIST_ROUTE) {
      return pathname === path;
    } else if (path === C_ALL_INVENTORY_ROUTE) {
      return pathname.includes(path);
    } else if (path === C_INVENTORY_FEED_DASHBOARD_ROUTE) {
      return pathname === path;
    }
    return false;
  };
