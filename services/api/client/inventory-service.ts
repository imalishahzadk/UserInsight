...
class InventoryService extends BaseService {
  async getInventoryFeed(): Promise<IInventoryFeed[]> {
    try {
      const res = await this.httpService.get("/clients/get-inventory-feed");
  ...
  async getInventoryFeedById(
    ...
    const res = await this.httpService.get(
      `/clients/get-inventory-feed-by-id?inventoryId=${inventoryId}`
    );
  ...
  async deleteInventoryFeed(inventoryId: string): Promise<ApiResponse> {
    try {
      const res = await this.httpService.get(
        `/clients/delete-inventory-feed?inventoryId=${inventoryId}`
      );
  ...
  async getInventorySummary(): Promise<IInventorySummary> {
    const res = await this.httpService.get(`/clients/get-inventory-summary`);
  ...
  async getInventoryItems(filters: any): Promise<InventoryResponse> {
    const res = await this.httpService.post(
      `/clients/get-all-inventory-items`,
      filters
    );
