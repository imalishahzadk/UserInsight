...
import { A_ADD_USER_ROUTE, A_UPDATE_USER_ROUTE } from "@/core/routes";
...
const UsersTable = ({ searchQuery }: UsersTableProps) => {
  const [users, setAllUsers] = useState<IUser[]>([]);
  ...
  //   const handleActionClick = (action: string, user: IUser) => {
  //     if (action === "Edit") {
      ...
      format: (value, row: IUser) => {
        return (
          <div className="flex justify-center items-center gap-2">
            {canEdit && (
              <Link
                href={replaceUrlVariables(A_UPDATE_USER_ROUTE, {
                  userId: row._id,
                })}
              >
  ...
  return (
    ...
    >
      <DataTable
        ...
        //   ...action,
        //   onClick: (row: IUser) => handleActionClick(action.label, row),
        // }))}
      />
    </Box>
  );
